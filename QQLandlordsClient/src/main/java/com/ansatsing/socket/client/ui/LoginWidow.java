...
import com.ansatsing.socket.util.PictureUtil;
			...
			public void mouseClicked(MouseEvent e) {
				if(userNameField.getText().equals("")) {
					...
					try {
						...
						printWriter.println("name-@"+userNameField.getText().trim());
						...
						if(readMsg.equals("这个网名可以啦!")) {//进入聊天室
							QQGameWindow qqGameWindow = new QQGameWindow(socket,userNameField.getText().trim());
							//System.exit(0);
							dispose();//仅仅关闭窗体
						}else {//重新输入网名
